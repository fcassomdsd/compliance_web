...
- `SITE_VISIT_SCHEDULING_JOB_HOUR` (default `2`) — local hour the daily sweep runs. It queries active `InspectionCadence` records (a recurring cadence per LocationService × Specialty × ActivityType, managed in AtroCore — the `LocationService` supplies both the location the visit happens at and the provider being inspected) for any with `nextDueDate` in the past, and for each one creates a `SiteVisit`, an `InspectedProvider` joining that visit to the cadence's provider, and an `Inspection` against that junction row, then advances the cadence's `lastScheduledDate`/`nextDueDate` by its `intervalMonths`. The junction row is not optional bookkeeping: `Inspection` declares no `siteVisit` field, so `Inspection → InspectedProvider → SiteVisit` is the only path from an inspection to its visit.
