...
3. `specialtyScope` is the set of specialty codes the session may see and act on, taken from the signed-in user's `Inspector` record (`externalUserID` = the Alfresco username) through Node-RED's `GET /inspector/:externalId`, which returns `specialties: [{ id, code, name }]`. The Inspector record is the single source of truth; nothing is duplicated onto the assignment groups, which would need keeping in sync.
   ...
   - It is stored in `auth_session.metadata_json` alongside `groups`, so no schema change was needed. `metadata.inspectorId` travels beside it — the id of the signed-in user's own `Inspector` record, resolved for **any** session holding the `inspector` role (not only a scoped one, since an inspector who is also a planner can still lead a site visit). It is what says whether this user is the main inspector of a given visit, which gates the report outcome; see [`AUTH_CHUNK1_ROUTE_AUTH_MATRIX.md`](AUTH_CHUNK1_ROUTE_AUTH_MATRIX.md) §5.
